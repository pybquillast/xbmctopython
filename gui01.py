...
class Window(object):
    ...
    def _setGUI(self, root):
        ...
        self.focus = grpRoot = ControlGroup(0, 0, width, height)
        grpRoot._isSelected = True
        grpRoot.master = self
        grpRoot._wdgID = 0
        self.wndTk = tk.Toplevel(root)
        # self.wndTk.overrideredirect(1)
        self.wndTk.transient(root)
        # self.wndTk.attributes('-fullscreen', True)
        canvas = grpRoot._createContiner(self.wndTk)
        canvas.pack()
    ...
    def addControls(self, controls):
        ...
        *Throws:
        ...
        for control in controls:
            container._registerControl(control)
    ...
    def setFocus(self, control=None):
        ...
        master._onEnter(None, control)
        # focusedWdg = self.getFocus()
        # if focusedWdg != self.focus:
        #     master = focusedWdg.master
        #     master._onLeave(None, focusedWdg)
...
class Control(object):
    ...
    def _setWdgImage(self):
        ...
        litem = self._wdgInfo
        pilImg = controlXml.getImageFromLayout(layout, litem)
        self.wdgImg = ImageTk.PhotoImage(pilImg)
        if hasattr(self, '_wdgID'):
...
class ControlGroup(Control):
    ...
    def _createContiner(self, parent, control=None):
    ...
    def _isContainer(self, control):
        return isinstance(control, ControlGroup)
    ...
    def _registerControl(self, control):
        ...
        control.master = master = control.master or self
        wdg = master.widget
        ...
        if self._isContainer(control):
            control.widget = wdg
            wdgID = wdg.create_image(xpos, ypos, anchor=tk.NW)
            control._xorig, control._yorig = xpos, ypos
        elif not hasattr(control, 'widget'):
            def onEnter(event, ctrl=control):
                return master._onEnter(event, ctrl)
            def onLeave(event, ctrl=control):
                return master._onLeave(event, ctrl)
            control._setWdgImage()
            wdgID = wdg.create_image(xpos, ypos, image=control.wdgImg, anchor=tk.NW)
            wdg.tag_bind(wdgID, '<Enter>', onEnter)
            wdg.tag_bind(wdgID, '<Leave>', onLeave)
        else:
            ctrlWdg = self._createContiner(wdg, control)
            wdgID = wdg.create_window(xpos, ypos, window=ctrlWdg, anchor=tk.NW, tags='container')
        tag1 = re.findall('\.([^ ]+) ', str(control))[0]
        tag2 = 'grp' + str(master._wdgID).rjust(3, '0')
        wdg.itemconfigure(wdgID, tags=(tag1, tag2))

        control._wdgID = wdgID
    ...
    def _onEnter(self, event, control):
        ...
        if ctrlgrp:
            while self._isContainer(ctrlgrp):
...
class ControlList(Control):
    ...
    def _registerControl(self, control):
        ...
        xpos, ypos = control.getPosition()
        control._setWdgImage()
        wdgID = wdg.create_image(xpos, ypos, image=control.wdgImg, anchor=tk.NW, tags='item')
    ...
    def _onEnter(self, event, ctrl):
        ...
        if not self._isSelected: self.master._onEnter(None, self)
        ...
        ctrl._setWdgImage()
        # wdg.itemconfigure(wdgID, image=ctrl._getEnabledImg())
